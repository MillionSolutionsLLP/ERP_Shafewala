        ...
        if($id){
         ...
         //   var_dump($code);
            if($code){
              ...
              //  $this->code1=$code;
                $this->table=Base::getTable($id).$code;}
...
public static function getWarehouseCode(){
        ...
        $model=new Model(0);
        $data=$model->pluck('WarehouseLocation','UniqId')->toArray();
        
        $return=[];
        $data2=$model->pluck('WarehouseName','UniqId')->toArray();

        foreach ($data2 as $key => $value) {
                   $return[$key]=  $value.",".$data[$key];
                }        
                return $return;
        return $model->pluck('WarehouseName','UniqId')->toArray();        
